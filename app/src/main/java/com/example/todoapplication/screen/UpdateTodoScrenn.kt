...
@Composable
fun UpdateTodoTopSection(navController: NavController, modifier: Modifier = Modifier) {
    Box(
        modifier = modifier,
...
fun UpdateTodoContent(
    ...
    var todoTitle by remember {
        mutableStateOf(state.todo)
    }
    var todoDescription by remember {
        mutableStateOf(state.shortDescription)
    }
    Column(
        ...
        state.todo = todoTitle
        state.shortDescription = todoDescription
        UpdateTodoPriorityContent(onEvent = onEvent, state = state)
        color = UpdateTodoDropDown(onEvent = onEvent, state = state)
        val todo = Todo(
            color.value.toLong(),
            state.todo,
            state.priority,
        ...
        Button(
            onClick = {
                try {
                    onEvent(TodoEvent.UpdateTodos(todo))
...
fun UpdateTodoPriorityContent(
    ...
    var selectedOption by remember {
        mutableIntStateOf(state.priority)
    }
    Row {
        priority.forEach {
            Row(
    ...
    state.priority = selectedOption
}

@Composable
fun UpdateTodoDropDown(
    modifier: Modifier = Modifier,
    onEvent: (TodoEvent) -> Unit,
    state: Todo
): Color {
    ...
    var mSelectedText by remember { mutableStateOf(Color(state.color.toULong())) }
    ...
    Column(modifier = modifier.padding(20.dp)) {
        ...
        OutlinedTextField(
            value = convertColorName(mSelectedText.toString()),
            onValueChange = {
